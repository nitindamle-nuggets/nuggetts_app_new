...
<div class="container">
    ...
    <div class="section">
        ...
        <div id="departmentRecords" class="tab-content">
            <div class="section">
                                ...
                                <tbody>
                                    @forelse ($departments as $department)
                                        <tr>
                                            ...
                                            <td>{{ $department->created_at->format('Y-m-d') }}</td>
                                            <td>
                                                <a href="{{ route('admin.departments.edit', $department->id) }}" class="action-btn action-btn-edit">Edit</a>
                                                <button type="button" class="action-btn action-btn-delete" onclick="confirmDelete({{ $department->id }}, '{{ $department->name }}')">Delete</button>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="4" style="text-align:center; padding:20px;">No departments found</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
...
<div id="deleteModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); z-index:1000; justify-content:center; align-items:center;">
    <div style="background:#fff; padding:30px; border-radius:8px; max-width:400px; box-shadow:0 4px 6px rgba(0,0,0,0.1);">
        <h2 style="margin:0 0 15px; color:#1e3c72;">Confirm Delete</h2>
        <p id="deleteMessage" style="margin:15px 0; color:#666;"></p>
        <div style="display:flex; gap:10px; margin-top:25px;">
            <button onclick="cancelDelete()" class="btn btn-secondary" style="flex:1; margin:0;">Cancel</button>
            <form id="deleteForm" method="POST" style="flex:1;">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-primary" style="width:100%; margin:0;">Delete</button>
            </form>
        </div>
    </div>
</div>
...
function confirmDelete(departmentId, departmentName) {
    document.getElementById('deleteMessage').textContent = `Are you sure you want to delete the department "${departmentName}"? This action cannot be undone.`;
    document.getElementById('deleteForm').action = "{{ route('admin.departments.index') }}/" + departmentId;
